...
package com.asuka.android.asukaandroid.http;

import android.text.TextUtils;

import com.asuka.android.asukaandroid.comm.utils.LogUtil;
import com.asuka.android.asukaandroid.http.client.entity.BodyParamsEntity;
import com.asuka.android.asukaandroid.http.client.multipart.HttpMultipartMode;
import com.asuka.android.asukaandroid.http.client.multipart.MultipartEntity;
import com.asuka.android.asukaandroid.http.client.multipart.content.ContentBody;
import com.asuka.android.asukaandroid.http.client.multipart.content.FileBody;
import com.asuka.android.asukaandroid.http.client.multipart.content.InputStreamBody;
import com.asuka.android.asukaandroid.http.client.multipart.content.StringBody;
import com.asuka.android.asukaandroid.http.task.Priority;
...
public class EGRequestParams {
    ...
    protected final ConcurrentHashMap<String, List<ContentBody>> fileArrayParams = new ConcurrentHashMap<String, List<ContentBody>>();

    private Priority priority;

    public EGRequestParams() {
    }
    ...
    public EGRequestParams(String charset) {
    ...
    public void addBodyParameter(String key, File files[]) throws FileNotFoundException {
        if (key != null) {
            List<ContentBody> fileWrappers = new ArrayList<ContentBody>();
            for (File file : files) {
                if (file == null || !file.exists()) {
                    throw new FileNotFoundException();
                }
                fileWrappers.add(new FileBody(file));
            }
    ...
    public HttpEntity getEntity() {
        ...
        if ((fileArrayParams!=null&&!fileArrayParams.isEmpty())||(fileParams != null && !fileParams.isEmpty())) {
            ...
            if(fileArrayParams!=null&&!fileArrayParams.isEmpty()) {
                // Add file collection
                for (ConcurrentHashMap.Entry<String, List<ContentBody>> entry : fileArrayParams.entrySet()) {
                    List<ContentBody> fileWrapper = entry.getValue();
                    for (ContentBody fw : fileWrapper) {
                        multipartEntity.addPart(entry.getKey(), fw);
                    }
                }
            }
